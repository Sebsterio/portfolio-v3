		...
		<div className={cn('flex gap-4', className)}>
			{views.map(({ mode, label }) => (
				<TransitionLink
					...
					className={cn(
						'transition-duration-300 rounded-full px-6 py-3 font-dm-sans text-sm font-semibold transition-colors',
						currentMode === mode ? 'gradient-primary text-white' : 'text-muted bg-surface-glass-sm hover:bg-surface-glass-lg hover:text-chrome-silver',
					)}
				>
					{label}
				</TransitionLink>
